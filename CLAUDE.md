...
A dine-in/takeaway extension: per-line destination (Mesa/Para llevar), free-text kitchen notes, free modifiers, configurable combos ("Completo" = pick one item from each of N slots at a fixed price), kitchen ticket (comanda) printing over ESC/POS, and combo-aware reports. Gated behind `Restaurant Settings.enable_restaurant_mode` (off by default) — every restaurant UI surface is conditional on `restaurantStore.enabled`, and disabled sites see zero behavior change.

- **Doctypes** (`pos_prime/pos_prime/doctype/restaurant_*`, listed in the exception noted above): `Restaurant Combo` (+ `Restaurant Combo Slot`), `Restaurant Modifier` (+ `Restaurant Modifier Group` and its two Table MultiSelect child doctypes), `Restaurant Printer`, `Restaurant Settings` (Single), and the transactional `Restaurant Order` (+ `Restaurant Order Combo`/`Item`/`Modifier`), submitted alongside its POS Invoice inside `create_restaurant_sale` — never before. `Restaurant Order Item.combo_uid` is what the whole reporting layer depends on: empty means sold individually, set means it's one component of the combo instance sharing that uid.
- **Combo pricing is backend-only and authoritative** (`pos_prime/restaurant/pricing.py::distribute_combo_price`) — splits a combo's fixed price across its components' list prices using the largest-remainder method in integer minor units, so the split always sums to exactly the combo price. The frontend only ever previews a split via `preview_combo`; `create_restaurant_sale` recomputes from scratch server-side regardless of what the client sent.
