...
def lock_combo_rates(invoice, locked_rates):
    """Pin the given item rows' rates so ERPNext can't re-derive them.

    Restaurant combo components are priced by distribute_combo_price (see
    pos_prime/restaurant/pricing.py), not by the item's price list, so
    set_missing_values() would happily overwrite them with the list rate.
    AccountsController.validate() calls set_missing_values() (possibly more
    than once — e.g. again on submit), so wrap the bound method rather than
    fixing rates once: Python resolves the instance attribute before the
    class method, so every internal call is caught.  price_list_rate must
    be locked too, or calculate_item_values() resets rate from it when
    rate == 0 (a legitimately free combo component).

    locked_rates maps the row's index in invoice.items to its rate.
    """
    if not locked_rates:
        return
